...
Create or choose a folder for this company's books. For example, use a folder
named after the company inside Documents, such as `Documents/books/acme-co`.
Do not use the `/books` plugin source folder as the company books folder.

Open Claude Code, Claude Cowork, or Codex in that folder, enable the `books`
plugin, and say:
...
hand. The agent should translate plain-English corrections into deterministic CLI
commands.
