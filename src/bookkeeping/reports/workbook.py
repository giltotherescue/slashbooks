    ...
    if store_path.exists():
        try:
            ...
            return SanityCheck(
                check="currency_scope",
                status="warn",
                detail=f"Could not inspect ledger store currencies: {exc}",
            )
    else:
...
def _parse_all_entries(entity_path: Path) -> list[Any]:
    ...
    if store_path.exists():
        try:
            return LedgerStore(store_path).load_entries()
        except Exception as exc:
            raise ValueError(f"Could not read canonical ledger store: {exc}") from exc
    books_path = entity_path / "books.beancount"
...
def generate_accountant_package(
    ...
    if gl_from_date > gl_to_date:
        return PackageResult(
            output_dir=Path(output_dir) if output_dir is not None else entity_path / "reports" / "accountant-export" / f"{from_date}_{to_date}",
            sanity=SanityResult(checks=[]),
            error="--gl-from must be on or before --gl-to",
        )
    ...
    summary_formulas = summary_formula_deps.issubset(selected_set)
    checks_formulas = checks_formula_deps.issubset(selected_set)

    rows_cache: dict[str, list[list[str]]] = {}
    xlsx_cache: dict[str, XlsxSheet] = {}
    entries_unloaded = object()
    entries_cache: list[Any] | None | object = entries_unloaded
    entity_config_cache: dict[str, Any] | None = None

    def _statement_rows(key: str, compute: Callable[[], StatementResult]) -> list[list[str]]:
        if key not in rows_cache:
            try:
                rows_cache[key] = _statement_to_rows(compute())
            except Exception as exc:
                raise RuntimeError(f"Could not compute {key}: {exc}") from exc
        return rows_cache[key]

    def _all_entries() -> list[Any] | None:
        nonlocal entries_cache
        if entries_cache is not entries_unloaded:
            return entries_cache  # type: ignore[return-value]
        try:
            entries_cache = _parse_all_entries(entity_path)
        except Exception:
            entries_cache = None
        return entries_cache

    def _entries_for_period(start: date, end: date) -> list[Any] | None:
    ...
    def _entity_config() -> dict[str, Any]:
        nonlocal entity_config_cache
        if entity_config_cache is None:
            entity_config_cache = _load_entity_config(entity_path)
        return entity_config_cache

    def _rows(key: str) -> list[list[str]]:
        if key in rows_cache:
            return rows_cache[key]
        if key == "cover":
            rows_cache[key] = _build_cover_rows(entity_path, entity_name, from_date, to_date, sanity, override_used)
        elif key == "pnl":
            return _statement_rows("pnl", lambda: profit_and_loss(entity_path, from_date, to_date))
        elif key == "balance-sheet":
            return _statement_rows("balance-sheet", lambda: balance_sheet(entity_path, to_date))
        elif key == "trial-balance":
            return _statement_rows("trial-balance", lambda: trial_balance(entity_path, to_date))
        elif key == "general-ledger":
            rows_cache[key] = _build_general_ledger_rows(
                entity_path,
                gl_from_date,
                gl_to_date,
                entries=_entries_for_period(gl_from_date, gl_to_date),
            )
        elif key == "reconciliations":
            rows_cache[key] = _build_reconciliation_rows(entity_path)
        elif key == "vendor-1099":
            rows_cache[key] = _build_vendor_1099_rows(
                entity_path,
                from_date,
                to_date,
                threshold_1099,
                entries=_entries_for_period(from_date, to_date),
            )
        elif key == "adjustment-log":
            rows_cache[key] = _build_adjustment_log_rows(entity_path, entries=_all_entries())
        elif key == "open-questions":
            rows_cache[key] = _build_open_questions_rows(entity_path)
        elif key == "source-index":
            rows_cache[key] = _build_source_index_rows(entity_path)
        elif key == "audit-log":
            rows_cache[key] = _build_audit_log_rows(entity_path)
        elif key == "summary":
            return _summary_rows(formulas=False)
        elif key == "checks":
            return _checks_rows(formulas=False)
        else:
            raise KeyError(key)
        return rows_cache[key]

    def _summary_rows(*, formulas: bool) -> list[list[str]]:
        cache_key = "summary:xlsx" if formulas else "summary:csv"
        if cache_key not in rows_cache:
            prior_bs_rows = _statement_rows(
                "prior-balance-sheet",
                lambda: balance_sheet(entity_path, from_date - timedelta(days=1)),
            )
            rows_cache[cache_key] = _build_summary_rows(
                entity_name,
                _entity_config(),
                from_date,
                to_date,
                ...
                gl_rows=_rows("general-ledger") if "general-ledger" in selected_set else None,
                recon_rows=_rows("reconciliations") if "reconciliations" in selected_set else None,
                vendor_rows=_rows("vendor-1099") if "vendor-1099" in selected_set else None,
                open_question_rows=_rows("open-questions") if "open-questions" in selected_set else None,
                formulas=formulas,
            )
        return rows_cache[cache_key]

    def _checks_rows(*, formulas: bool) -> list[list[str]]:
        cache_key = "checks:xlsx" if formulas else "checks:csv"
        if cache_key not in rows_cache:
            rows_cache[cache_key] = _build_checks_rows(
                _rows("pnl"),
                _rows("balance-sheet"),
                _rows("trial-balance"),
                _summary_rows(formulas=summary_formulas) if formulas else _summary_rows(formulas=False),
                formulas=formulas,
            )
        return rows_cache[cache_key]

    def _xlsx_spec(key: str) -> XlsxSheet:
        if key in xlsx_cache:
            return xlsx_cache[key]
        if key == "cover":
            spec = XlsxSheet("Cover", _rows(key), tab_color="#5B9BD5")
        elif key == "pnl":
            spec = XlsxSheet("P&L", _rows(key), tab_color="#A9D18E", numeric_columns=frozenset({2}))
        elif key == "balance-sheet":
            spec = XlsxSheet("Balance Sheet", _rows(key), tab_color="#A9D18E", numeric_columns=frozenset({2}))
        elif key == "trial-balance":
            spec = XlsxSheet("Trial Balance", _rows(key), tab_color="#A9D18E", numeric_columns=frozenset({1, 2}))
        elif key == "general-ledger":
            spec = XlsxSheet(
                "General Ledger",
                _rows(key),
            ...
            spec = XlsxSheet("Reconciliations", _rows(key), table=True, numeric_columns=frozenset({2, 3, 4}))
        elif key == "vendor-1099":
            spec = XlsxSheet("Vendor 1099", _rows(key), table=True, tab_color="#F4B183", numeric_columns=frozenset({1, 3, 4}))
        elif key == "adjustment-log":
            spec = XlsxSheet("Adjustment Log", _rows(key), table=True, numeric_columns=frozenset({3, 8}))
        elif key == "open-questions":
            spec = XlsxSheet("Open Questions", _rows(key), table=True, numeric_columns=frozenset({5}))
        elif key == "summary":
            spec = XlsxSheet(
                "Summary",
                _summary_rows(formulas=summary_formulas),
                table=True,
                tab_color="#70AD47",
                formula_columns=frozenset({1}) if summary_formulas else frozenset(),
                numeric_columns=frozenset({1}),
            )
        elif key == "checks":
            spec = XlsxSheet(
                "Checks",
                _checks_rows(formulas=checks_formulas),
                table=True,
                tab_color="#FFC000",
                formula_columns=frozenset({1, 2}) if checks_formulas else frozenset(),
                numeric_columns=frozenset({1}),
            )
        elif key == "source-index":
            spec = XlsxSheet("Source Index", _rows(key), table=True, numeric_columns=frozenset({2, 3, 4}))
        elif key == "audit-log":
            spec = XlsxSheet("Audit Log", _rows(key), table=True)
        else:
            raise KeyError(key)
        xlsx_cache[key] = spec
        return spec

    csv_names_by_key: dict[str, str] = {
        "cover": "Cover",
        "pnl": "P&L",
        "balance-sheet": "Balance Sheet",
        "trial-balance": "Trial Balance",
    ...
    try:
        sheet_data: list[tuple[str, list[list[str]]]] = [
            (csv_names_by_key[key], _rows(key)) for key in selected_sheet_keys
        ]
        xlsx_sheet_data: list[XlsxSheet] = [
            _xlsx_spec(key) for key in selected_sheet_keys
...
def run(args: Any) -> int:
    ...
    if cmd == "export":
        ...
        if from_date > to_date:
            ...
            return 1
        if gl_from_date and gl_to_date and gl_from_date > gl_to_date:
            print("Error: --gl-from must be on or before --gl-to", file=sys.stderr)
            return 1
