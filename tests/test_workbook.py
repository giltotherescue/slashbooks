...
class TestAllCSVsProduced(unittest.TestCase):
    ...
    def test_selected_light_sheets_do_not_build_omitted_schedules(self):
        from src.bookkeeping.reports import workbook
        from src.bookkeeping.reports.workbook import generate_accountant_package

        clean_sanity = workbook.SanityResult(checks=[])
        with patch.object(workbook, "run_sanity_checks", return_value=clean_sanity), \
            patch.object(workbook, "_parse_all_entries", side_effect=AssertionError("parsed entries")), \
            patch.object(workbook, "_build_general_ledger_rows", side_effect=AssertionError("built GL")), \
            patch.object(workbook, "_build_vendor_1099_rows", side_effect=AssertionError("built vendor")), \
            patch.object(workbook, "_build_adjustment_log_rows", side_effect=AssertionError("built adjustments")), \
            patch.object(workbook, "_build_source_index_rows", side_effect=AssertionError("built source index")), \
            patch.object(workbook, "_build_audit_log_rows", side_effect=AssertionError("built audit")):
            result = generate_accountant_package(
                self.entity_dir,
                FROM_DATE,
                TO_DATE,
    ...
    def test_audit_log_export_does_not_build_financial_schedules(self):
        ...
        clean_sanity = workbook.SanityResult(checks=[])
        with patch.object(workbook, "run_sanity_checks", return_value=clean_sanity), \
            patch.object(workbook, "profit_and_loss", side_effect=AssertionError("computed pnl")), \
            patch.object(workbook, "balance_sheet", side_effect=AssertionError("computed bs")), \
            patch.object(workbook, "trial_balance", side_effect=AssertionError("computed tb")), \
            patch.object(workbook, "_parse_all_entries", side_effect=AssertionError("parsed entries")):
            result = generate_accountant_package(
                self.entity_dir,
                FROM_DATE,
        ...
        self.assertIsNone(result.error, f"Package generation failed: {result.error}")
        self.assertEqual([p.name for p in result.csv_files], ["Audit-Log.csv"])
    ...
    def test_invalid_general_ledger_period_returns_error(self):
        from src.bookkeeping.reports.workbook import generate_accountant_package
        ...
        result = generate_accountant_package(
            ...
            override=True,
            sheets="general-ledger",
            gl_from_date=date(2026, 4, 1),
            gl_to_date=date(2026, 3, 1),
        )

        self.assertFalse(result.success)
        self.assertIn("--gl-from", result.error or "")
...
class TestXlsxCreatedWhenPresent(unittest.TestCase):
    ...
    def test_xlsx_summary_without_dependency_tabs_uses_plain_values(self):
        from src.bookkeeping.reports.workbook import generate_accountant_package
    ...
    def test_xlsx_checks_without_review_tabs_uses_plain_values(self):
        from src.bookkeeping.reports.workbook import generate_accountant_package
        ...
        result = generate_accountant_package(
            ...
            TO_DATE,
            override=True,
            sheets="pnl,balance-sheet,trial-balance,summary,checks",
        )
        self.assertIsNotNone(result.xlsx_file)
