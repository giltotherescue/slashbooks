...
- Company directories contain `entity.json`, `books.beancount`, review queues,
  learned context, audit logs, cache files, and generated reports.
- Plugin upgrades must not overwrite company data. Entity onboarding copies
  templates one way into the company directory.
...
To add a reusable connector or a chart-of-accounts template, follow the recipes
in [CONTRIBUTING.md](CONTRIBUTING.md). A one-off, company-specific connector
