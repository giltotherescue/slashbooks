    ...
    if dest.exists() and not force:
        ...
        try:
            ...
            return MigrationResult(
                ...
                dry_run=dry_run,
                ledger_sha256=ledger_sha,
                error_message=f"Store already exists: {dest}. Re-run with --force to replace it.",
            )
            ...
            return MigrationResult(
                ...
                ledger_sha256=ledger_sha,
                error_message=(
                    f"Existing store could not be inspected: {dest}. "
                    f"Re-run with --force to replace it. ({exc})"
                ),
            )
        ...
        with store.transaction() as conn:
            ...
            store.append_audit_event(
                "ledger-store-sealed",
                {
                    ...
                    "entries": len(parsed["entries"]),
                    "source_ids": [entry.source_id or "" for entry in parsed["entries"]],
