...
class TestImportTransactionsBasic(unittest.TestCase):
    ...
    def test_duplicate_source_ids_in_same_batch_are_skipped(self) -> None:
        txns = [
            _make_posted_txn("txn-batch-dup", amount="12.00"),
            _make_posted_txn("txn-batch-dup", amount="12.00"),
        ]
        ...
        result = import_transactions(
            ...
            txns,
            SESSION,
            categorizer=_simple_categorizer("Expenses:Meals"),
            ts=TS,
            session_date=date(2026, 1, 15),
        )
        ...
        self.assertEqual([e.source_id for e in entries], ["txn-batch-dup"])
...
class TestAtomicWriteAndIntegrity(unittest.TestCase):
    ...
    def test_store_content_tamper_returns_halt(self) -> None:
        txns = [_make_posted_txn("txn-content-tamper", amount="10.00")]
        import_transactions(
            self._entity, txns, SESSION,
            categorizer=_simple_categorizer("Expenses:Services"),
            ts=TS,
        ...
        self.assertEqual(result.status, "halt")
        self.assertIn("sealed digest", result.message)

    def test_missing_terminal_seal_returns_incomplete_write(self) -> None:
        txns = [_make_posted_txn("txn-missing-seal")]
        import_transactions(
            self._entity, txns, SESSION,
            categorizer=_simple_categorizer("Expenses:Services"),
            ts=TS,
    ...
    def test_audit_log_contains_intent_and_seal(self) -> None:
        ...
        seal_payload = records[seal_idx]["payload"]
        self.assertRegex(seal_payload.get("store_sha256", ""), r"^[0-9a-f]{64}$")

    def test_transaction_failure_rolls_back_store_and_seen_ids(self) -> None:
        def fail_after_entries(self, txns, conn, *, status="posted", imported_at=None):
            raise RuntimeError("simulated source payload failure")

        with patch.object(LedgerStore, "insert_source_transactions", fail_after_entries):
            with self.assertRaises(RuntimeError):
                import_transactions(
                    self._entity,
                    [_make_posted_txn("txn-rollback")],
                    SESSION,
                    categorizer=_simple_categorizer("Expenses:Services"),
                    ts=TS,
                    session_date=date(2026, 1, 15),
        ...
        self.assertEqual(store.counts().postings, 0)
        self.assertEqual(store.counts().audit_events, 0)
        self.assertFalse(StagingStore(self._entity.staging_dir).is_seen("txn-rollback"))
