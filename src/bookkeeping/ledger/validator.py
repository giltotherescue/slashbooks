...
_POSTING_META_RE = re.compile(r'^\s{4}([a-z][a-z0-9\-_]*):\s+"((?:[^"\\]|\\.)*)"$')
...
class _ParsedEntry:
    ...
    postings: list[tuple[str, Decimal, str, list[tuple[str, str]]]]
...
def parse_ledger(text: str) -> dict:
    ...
    def flush_entry() -> None:
        ...
        current_entry = None
        postings_out = []
        for acc, amt, cur, meta in pe.postings:
            postings_out.append(
                Posting(
                    account=acc,
                    amount=amt,
    ...
    while i < n:
        ...
        if current_entry is not None:
            ...
            pmm = _POSTING_META_RE.match(raw)
            if pmm and current_entry.postings:
                current_entry.postings[-1][3].append((pmm.group(1), _unescape(pmm.group(2))))
                continue
...
def validate(text: str) -> list[ValidationError]:
    ...
    for block_line, pe in txn_blocks:
        # Zero-sum check
        total = sum(amt for _, amt, _, _ in pe.postings)
        ...
        for acc, amt, cur, _meta in pe.postings:
            if acc not in opened_accounts:
                errors.append(
                    ValidationError(
                        block_line,
